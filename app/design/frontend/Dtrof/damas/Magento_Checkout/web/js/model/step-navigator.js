    ...
    function($, ko) {
        ...
        return {
            ...
            navigateTo: function(code, scrollToElementId) {
                ...
                var active  = $('#checkoutSteps').find('li.active');
                active.removeClass('active');
                sortedItems.forEach(function(element) {
                    if (element.code == code) {
                        element.isVisible(true);
                        $('#' + code).addClass('active');
                        bodyElem.animate({scrollTop: $('#' + code).offset().top}, 0);
                        element.navigate();
            ...
            next: function() {
                var activeIndex = 0;
                //stepsSorted = steps.sort(this.sortItems);
                steps.sort(this.sortItems).forEach(function(element, index) {
                ...
                if (steps().length > activeIndex + 1) {
                    ...
                    var prev = steps()[activeIndex].code;
                    if (code == 'shipping') {
            ...
            prev: function(){
                var activeIndex = 0;
                steps.sort(this.sortItems).sort(this.sortItems).forEach(function(element, index) {
                    if (element.isVisible()) {
                        element.isVisible(false);
                        activeIndex = index;
                    }
